...
// For now, using the working v1 implementation in generate-carousel-long-image.ts
...
export async function generateCarouselLongImageV2(
  ...
  throw new Error("V2 implementation temporarily disabled - use generateCarouselLongImage instead");
}
...
function generateSeedFromPrompt(prompt: string, slideCount: number): number {
  let hash = 0;
  const combinedInput = `${prompt}_slides_${slideCount}`;
