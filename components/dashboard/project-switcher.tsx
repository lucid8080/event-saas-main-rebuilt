    ...
    <div>
      <Popover open={openPopover} onOpenChange={setOpenPopover}>
        <PopoverTrigger>
          ...
          >
            <div className="flex pr-2 space-x-3 items-center">
              <div
                className={cn(
    ...
    <div className="flex flex-col gap-1">
      {projects.map(({ slug, color }) => (
        ...
        >
          ...
          {selected.slug === slug && (
            <span className="absolute flex pr-3 text-foreground inset-y-0 right-0 items-center">
      ...
      <Button
        variant="outline"
        className="relative flex h-9 p-2 items-center justify-center gap-2"
        onClick={() => {
          setOpenPopover(false);
        }}
      >
        <Plus size={18} className="absolute left-2.5 top-2" />
        <span className="flex-1 text-center truncate">New Project</span>
      </Button>
    </div>
  );
}
...
function ProjectSwitcherPlaceholder() {
  return (
    <div className="flex px-1.5 py-2 space-x-1.5 rounded-lg sm:w-60 animate-pulse items-center">
      <div className="w-36 h-8 bg-muted rounded-md xl:w-[180px] animate-pulse" />
    </div>
  );
}
